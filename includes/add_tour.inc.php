...
if(isset($_POST['submit'])) {
    ...
    if ($conn) {
        ...
        if ($tour_id != -1) {
            ...
            $time = date('c');
            $sql1 = "UPDATE tours SET country = $country, category = $category, type = $type, price = $price, currency = $currency, quantity_adult = $q_adult,
                                                    quantity_child = $q_kid, quantity_small = $q_small, food_options = $food_option, hotel_stars = $hotel_stars WHERE tour_id = $tour_id";

            if(isset($_POST['rec']) && $_POST['rec'] == 'true') {
                $sql1 = "UPDATE tours SET country = $country, category = $category, type = $type, price = $price, currency = $currency, quantity_adult = $q_adult,
                                                    quantity_child = $q_kid, quantity_small = $q_small, food_options = $food_option, hotel_stars = $hotel_stars,
                                                     is_recommended = 1 WHERE tour_id = $tour_id";
            } elseif (isset($_POST['rec']) && ($_POST['rec'] != 'true')) {
                $sql1 = "UPDATE tours SET country = $country, category = $category, type = $type, price = $price, currency = $currency, quantity_adult = $q_adult,
                                                    quantity_child = $q_kid, quantity_small = $q_small, food_options = $food_option, hotel_stars = $hotel_stars,
                                                     is_recommended = 0 WHERE tour_id = $tour_id";
            }

            if (mysqli_query($conn, $sql1)) {
                $sql = "DELETE FROM tour_content WHERE tour_id = $tour_id";
                foreach ($languages as $language) {
                    $suffix = $language['keyword'];
                    $tour_name = mysqli_real_escape_string($conn, $_POST["tour_name_$suffix"]);
                    ...
                    if (!empty($tour_name) || !empty($tour_description)) {
                        $sql3 = "INSERT INTO tour_content (tour_id, tour_name, tour_description, tour_cities, hotel_name, language_key, tour_intro) VALUES 
                                                  ($tour_id, '$tour_name', '$tour_description', '$cities', '$hotel_name', '$suffix', '$tour_intro')";
    ...
    header("Location: ../admin2.php?message=შეცდომა");
    exit();
}